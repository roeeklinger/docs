...
description: "ByteStack authenticates every request with API keys. Learn how to generate keys, set the correct scope, pass them as Bearer tokens, and handle auth errors."
---
...
1. Open **Settings** → **API Keys** in the ByteStack dashboard.
2. Click **New Key**.
...
response = requests.post(
    "https://api.bytestack.dev/v1/queries",
    headers={
...
<Note>
  A `402 Payment Required` response means your account has an unpaid balance. ByteStack provides a 7-day grace period for failed charges. Resolve billing in **Settings** → **Plans & Billing** to restore full API access.
</Note>
