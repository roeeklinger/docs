...
description: "Configure webhooks to receive real-time HTTP callbacks when ByteStack jobs complete. Includes payload format, security verification, and retry behavior."
---
...
ByteStack signs each request using HMAC-SHA256 with your webhook secret. The signature is sent in the `X-ByteStack-Signature` header in the format `sha256=<hex_digest>`.
...
app.use('/webhooks/bytestack', express.raw({ type: 'application/json' }));
...
app.post('/webhooks/bytestack', (req, res) => {
  const signature = req.headers['x-bytestack-signature'] || '';
