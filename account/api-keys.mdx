---
title: "API keys"
description: "Generate, rotate, and revoke ByteStack API keys. Learn key scopes, best practices for securing credentials, and how to use keys in requests."
---
...
client = httpx.Client(
    base_url="https://api.bytestack.dev",
...
const response = await fetch("https://api.bytestack.dev/v1/queries", {
  method: "POST",
  headers: {
...
func main() {
	...
	req, _ := http.NewRequest("POST", "https://api.bytestack.dev/v1/queries", bytes.NewBuffer(body))
...
<Note>
  If you suspect an API key has been compromised, revoke it immediately and create a replacement. Then audit the affected services to confirm no unauthorized requests were made. Contact [legal@bytestack.dev](mailto:legal@bytestack.dev) if you believe your account was accessed without authorization.
</Note>
